...
# RUN fe40
WORKDIR /home/foam/foam-extend-4.0/ThirdParty/rpmBuild/SPECS
RUN sed -i '143s/--disable-mpi-f90/--enable-mpi-f90/' openmpi-1.8.8.spec && sed -i '143 a\        FC=gfortran \\' openmpi-1.8.8.spec
WORKDIR /home/foam/foam-extend-4.0/src/finiteVolume/finiteVolume/fvSchemes
RUN sed -i '382,394 s/^\(\s*\)\/\/ /\1/' fvSchemes.C
WORKDIR /home/foam/foam-extend-4.0/ThirdParty
...
WORKDIR /home/foam/root-4.0/FluidSolidInteraction/src
RUN source /home/foam/foam-extend-4.0/etc/bashrc && ./Allwmake > FSI_log.txt 2>&1
WORKDIR /home/foam/root-4.0/FluidSolidInteraction
...
WORKDIR /home/ParaFEM/parafem/build
RUN sed -i 's/FC=\/usr\/bin\/mpif90/FC=mpif90/' linuxdesktop.inc
WORKDIR /home/ParaFEM/parafem/include
RUN mkdir bem_p
WORKDIR /home/ParaFEM/parafem
RUN source /home/foam/foam-extend-4.0/etc/bashrc && ./make-parafem MACHINE=linuxdesktop > FSI_log.txt 2>&1 && echo "export PATH=~/ParaFEM/parafem/bin:$PATH" >> $HOME/.bashrc && source $HOME/.bashrc
WORKDIR /home
RUN git clone https://github.com/SPHewitt/OpenFPCI.git
ENV PARAFEM_DIR=/home/ParaFEM/parafem
